#!/bin/bash
#SBATCH --job-name=s1_full_surface_oracle_constant_no_pointmap_dropout
...
/n/holylabs/qianqian_lab/Lab/mwakeham/.conda/envs/sam3d-objects/bin/torchrun --standalone --nproc_per_node=4 train.py \
  ...
  --output-dir outputs/objaverse/conditioning_investigation/stage1_full_surface_oracle_constant_no_pointmap_dropout \
  --batch-size 4 \
  --workers 8 \
  --val-workers 2 \
  --epochs 20 \
