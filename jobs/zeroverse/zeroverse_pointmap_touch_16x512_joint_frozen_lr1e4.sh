#!/bin/bash
#SBATCH --job-name=zv_pointmap_touch_16x512_joint_frozen_lr1e4
...
/n/holylabs/qianqian_lab/Lab/mwakeham/.conda/envs/sam3d-objects/bin/torchrun --standalone --nproc_per_node=4 train.py \
  ...
  --data-config configs/data_zeroverse_5000_8views_touch_16x512.yaml \
  --output-dir outputs/zeroverse/zeroverse_pointmap_touch_16x512_joint_frozen_lr1e4 \
  --batch-size 4 \
  --workers 8 \
  --val-workers 2 \
