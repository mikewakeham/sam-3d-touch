...
/n/holylabs/qianqian_lab/Lab/mwakeham/.conda/envs/sam3d-objects/bin/torchrun --standalone --nproc_per_node=4 train.py \
  ...
  --output-dir outputs/objaverse/stage1_full_surface_no_pointmap_full_cross_attention \
  --batch-size 4 \
  --workers 8 \
  --val-workers 2 \
  --epochs 20 \
  ...
  --no-pointmap \
  --resume outputs/objaverse/stage1_full_surface_no_pointmap_full_cross_attention/last.pt \
  --wandb-id nouqb3mh
