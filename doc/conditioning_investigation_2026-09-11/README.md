...
**Latest result — view transfer fails despite stable oracle surfaces:** [VIEW_TRANSFER_FINDINGS.md](VIEW_TRANSFER_FINDINGS.md) validates exact anchor reproduction and matched new-view probes. Oracle loss stays at 0.01887 when only same-object surface view changes, but rises to 0.12771 when image/pointmap changes with the surface fixed. The previous successful fit is therefore not robust view-independent surface use. This does not establish failure to train on multiple views. The next prepared experiment directly fits four views per object and reserves three further views, retaining the same trainable modules and total object exposures.
