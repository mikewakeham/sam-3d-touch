...
**Current scope:** continue the existing coordinate experiments, retaining VecSetX. The prepared [alignment-tolerance probe](ALIGNMENT_TOLERANCE_HANDOFF.md) reuses the original and visual-dropout oracle checkpoints to establish their actual Stage-1 target agreement and sensitivity to residual pose error. No training; GPU results pending. [CONTEXT_AND_COORDINATE_NEXT_STEP.md](CONTEXT_AND_COORDINATE_NEXT_STEP.md) contains the paper/code/history review. Its full-dataset inventory proposal is optional, not a prerequisite. Older next-step sections below are historical unless explicitly activated against the pivotal findings.
