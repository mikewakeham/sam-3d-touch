...
**Next short GPU intervention — camera/oracle visual dropout:** [VISUAL_DROPOUT_HANDOFF.md](VISUAL_DROPOUT_HANDOFF.md) implements the two 1000-update runs motivated by the returned visual-stream probe. Historical baselines are replayed and reevaluated on fresh draws; initialization is restored before new training. Dropout is balanced within view groups, and actual shape-attention context is checked. Primary assessment retains all inputs. Local checks pass; GPU replay/training is pending. No full-dataset run.
