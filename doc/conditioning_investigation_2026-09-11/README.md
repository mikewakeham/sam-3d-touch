...
**Only next GPU action — constructive separate-attention pilot:** [SEPARATE_SURFACE_HANDOFF.md](SEPARATE_SURFACE_HANDOFF.md) adds an independent zero-output surface-attention branch around the frozen completed image model, retaining VecSetX. Four matched 1024-update arms compare oracle, constant and camera surfaces under that interface against joint oracle with the same image warm start. Exact image fallback and geometry-specific benefit over constant are required. Local checks pass; GPU execution is pending. No full training.
