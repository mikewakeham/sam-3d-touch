...
**Next GPU check — residual failure with aligned surfaces:** [ORACLE_VISUAL_STREAMS_HANDOFF.md](ORACLE_VISUAL_STREAMS_HANDOFF.md) uses the existing multi-view oracle checkpoint to separate RGB, silhouette and pointmap dependence with the oracle surface fixed. It includes historical endpoint replay, all eight stream combinations and wrong-object surface controls. No training or decoding. Local syntax/analyzer checks pass; GPU execution is pending. This tests the limit already visible with perfect alignment before investing in a learned pose correction.
