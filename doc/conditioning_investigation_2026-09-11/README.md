...
**Read before every next experiment:** [PIVOTAL_FINDINGS.md](PIVOTAL_FINDINGS.md) is the user's requested living ledger of the findings that materially narrow the diagnosis. It specifies each comparison, reference endpoint, scope, counterevidence and consequence for experiment design. Update it when pivotal evidence changes; distinguish improving a metric from closing a measured gap and from solving reconstruction.

**Current plan after the requested full evidence audit:** [EVIDENCE_AUDIT_AND_BRANCH_PLAN.md](EVIDENCE_AUDIT_AND_BRANCH_PLAN.md) reviews all completed experiment families and separates native-objective fidelity, generated shape up to pose, and new-object surface utility. It selects controlled CPU reanalysis of existing saved predictions before any new training. It also restores the early geometry finding as F0, qualifies the loss-only transfer claims, and records reuse of the 32-object development pool. Augmentation, changed target frame, practical pose recovery and broader-data/learning interventions are conditional branches, not concurrent jobs.

**Ready CPU artifact handoff:** [POSE_SHAPE_ARTIFACT_HANDOFF.md](POSE_SHAPE_ARTIFACT_HANDOFF.md) provides the actual interactive command to bundle all saved occupancy predictions and reports, plus natural-camera predictions if available. Helper transport checks passed locally. No new inference/training; actual arrays are still needed before pose-adjusted analysis can run.

**Latest interpretation correction:** F10's rotation sensitivity is measured in the fixed target frame; pose-only output changes versus actual shape damage have not been separated. Inspect the existing saved predictions for that distinction before launching the augmentation candidate below. Aligning a GT target to a prediction for diagnosis does not supply a deployment-time pose correction.

**Current scope:** continue the existing coordinate experiments, retaining VecSetX. [Alignment-tolerance results](ALIGNMENT_TOLERANCE_RETURNED_FINDINGS.md) now establish 97.17% reserved-view Stage-1 target IoU for oracle plus dropout, versus original oracle 66.95%, on four fitted identities. Some 5° residual rotations fail the prespecified tolerance; no tested nonzero angle passes all directions. Raw shards 0/1/3 passed local validation; shard 2 is missing from the attachments, while both supplied aggregates are identical. No rerun is needed. A matched short residual-rotation augmentation test is the next repair candidate, not yet implemented or launched. F6's transfer failure remains unresolved, and no full run is earned. [CONTEXT_AND_COORDINATE_NEXT_STEP.md](CONTEXT_AND_COORDINATE_NEXT_STEP.md) contains the paper/code/history review. Its full-dataset inventory proposal is optional, not a prerequisite. Older next-step sections below are historical unless explicitly activated against the pivotal findings.

**Latest result — separate attention is not a transfer fix:** [SEPARATE_SURFACE_RETURNED_FINDINGS.md](SEPARATE_SURFACE_RETURNED_FINDINGS.md) validates all four completed runs. Exact image fallback is preserved, but held-object losses are .13854 separate oracle, .14994 constant and .14072 camera versus image .11930 and warm-started joint oracle .12257. Correct-surface dependence is strong on fitted identities and fails on held identities. The candidate fails its acceptance criterion; do not extend or scale it.

**Paused GPU handoff:** [FULL_DATASET_CONDITIONING_HANDOFF.md](FULL_DATASET_CONDITIONING_HANDOFF.md) prepares an image/camera utility probe. It is not the current coordinate experiment, and the historical checkpoint inventory is not a prerequisite for continuing the established coordinate controls.

**Latest result — a constant surface reproduces fitting gains and transfer failure:** [CONSTANT_SURFACE_RETURNED_FINDINGS.md](CONSTANT_SURFACE_RETURNED_FINDINGS.md) validates the completed matched control. Giving every example the same training surface yields fit .05382, held-view .05639 and held-object .12790; real oracle is .05647/.05718/.13420 and image held-object is .11930. The constant also has the best early held-object mean. Extra token adaptation can reproduce the main pattern without sample-specific full-surface information; real geometry has not earned convincing transferable benefit.

**Completed separate-attention pilot:** [SEPARATE_SURFACE_HANDOFF.md](SEPARATE_SURFACE_HANDOFF.md) records the four matched 1024-update arms. All logging-fix results returned and passed validation. Do not rerun or extend this candidate; see the latest finding above.

**Latest result — surface-token presence is crucial, average identity benefit is small:** [TRANSFER_CHECKPOINTS_RETURNED_FINDINGS.md](TRANSFER_CHECKPOINTS_RETURNED_FINDINGS.md) validates all three early/late probes and exact historical replay. At step 256, wrong surfaces retain the advantage over image; at 1024, camera/oracle correct losses are .12208/.12965 versus image .11438. Removing the stream gives about .44 at both checkpoints. This supports a generic adaptation-pathway hypothesis, not proof of zero geometry use or catastrophic forgetting. Coordinate alignment alone is insufficient.

**Completed constant-surface control:** [CONSTANT_SURFACE_HANDOFF.md](CONSTANT_SURFACE_HANDOFF.md) records the matched sample-independent training control. Results have returned and passed validation; do not rerun it. See the latest finding above.
...
**Completed frozen checkpoint probe:** [TRANSFER_CHECKPOINTS_HANDOFF.md](TRANSFER_CHECKPOINTS_HANDOFF.md) records saved image/camera/oracle checkpoints at 256 and 1024, with exact historical replay and fresh held-object draws. All three reports have returned; do not rerun it. See the latest finding above.
