...
**Ready to run — coordinate factors:** [FRAME_FACTORS_HANDOFF.md](FRAME_FACTORS_HANDOFF.md) provides an interactive-terminal command for the two missing rotation/normalization treatments. The driver replays historical camera/oracle inputs, features, initial weights and initial losses before fitting; failed reproduction stops the job. VecSetX and the original adaptation scope remain unchanged. GPU execution is pending.

**Current plan — coordinate diagnosis resynchronized (12 September):** [COORDINATE_DIAGNOSIS_RESYNC.md](COORDINATE_DIAGNOSIS_RESYNC.md) reconciles the chat, completed probes, and source. Retain VecSetX and focus on full surfaces. The next contrast decomposes the measured camera/oracle benefit into orientation and view-dependent bbox/radius normalization; a new 64-sample CPU audit verifies that decomposition. The point-encoder replacement comparison is withdrawn and its unexecuted prototypes removed. Older plans below are historical and are not queued jobs.
