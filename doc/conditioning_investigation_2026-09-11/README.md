...
**Only next GPU action — one constant-surface training control:** [CONSTANT_SURFACE_HANDOFF.md](CONSTANT_SURFACE_HANDOFF.md) supplies every sample the same real training-surface feature bank for 1024 matched updates. It retains VecSetX, projector, token count, visuals and shared-attention optimization; the existing real baselines are reused. This removes sample-specific surface information from the start, avoiding the removal probe's distribution shift. Local checks pass; GPU results are pending. No full training or architecture replacement.
