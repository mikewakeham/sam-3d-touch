...
**Ready next — frozen spatial bridge control:** [SPATIAL_BRIDGE_HANDOFF.md](SPATIAL_BRIDGE_HANDOFF.md) tests saved native oracle surfaces through the exact target voxelizer and frozen Stage-1 VAE, before training another adapter. It compares against regenerated point-hit inputs, requires target reproduction, and measures a prespecified one-voxel sensitivity control. This separates spatial conversion from learning a new token-to-latent mapping; it is not a validated conditioner or sparse reconstruction solution. The handoff includes commands to paste into an allocated interactive terminal. GPU execution is pending.
