...
**Next bounded GPU training screen — 16 fitted / 16 held identities:** [OBJECT_TRANSFER_HANDOFF.md](OBJECT_TRANSFER_HANDOFF.md) implements image+pointmap, camera surface, oracle surface and oracle surface with dropout for 1024 updates from matched pretrained initialization, seed 37. Fitted views, held views and held objects are assessed separately with per-object losses and correct/wrong surface controls. Local protocol/analyzer checks pass; GPU execution is pending. Four allocated GPUs can run the arms concurrently. No full-dataset training.
