...
**Latest completed result (F12):** [Camera + dropout geometry](CAMERA_DROPOUT_GEOMETRY_RETURNED_FINDINGS.md) validates all 112 predictions and exact historical native replay. Reserved camera/dropout IoU is 56.17% versus oracle/dropout 97.17%; one-voxel shape F-score allowing the measured rigid transform is 79.26% versus 99.997%. Wrong surfaces do better on average. A reserved bathtub view produces the fitted shield in both draws (99.97%/99.89% raw F-score to the shield). This is a wrong-shape failure, not merely output pose. The inference-only handoff is completed; do not rerun it.

**Ready short coordinate intervention:** [SHARED_ORIENTATION_HANDOFF.md](SHARED_ORIENTATION_HANDOFF.md) provides the interactive command for one 1000-update camera-oriented-target arm. It preserves the existing camera inputs/VecSetX/features/schedule and reuses the historical fixed-target control. Target generation guards against clipping and gates VAE/common-unit fidelity before training; final correct/wrong-surface geometry is bundled automatically. Local checks passed, GPU execution pending. This tests an alternative target contract without committing the final product's frame. Overnight implementation is parked at the user's request; no production sources were changed.

**Read before every next experiment:** [PIVOTAL_FINDINGS.md](PIVOTAL_FINDINGS.md) is the user's governing living ledger. Read F0–F12 and their endpoint/scope limitations before choosing another branch. [EVIDENCE_AUDIT_AND_BRANCH_PLAN.md](EVIDENCE_AUDIT_AND_BRANCH_PLAN.md) preserves the earlier full audit; F11/F12 supersede its then-pending pose/shape and camera/dropout questions.

**Previous completed result (F11):** [Pose/shape analysis](POSE_SHAPE_RETURNED_FINDINGS.md) reproduces all 1,232 earlier saved predictions. Most 5° perturbations already preserve shape closely; some larger failures contain shape changes. All four alignment shards are now validated. Neither a universal sub-5° requirement nor a general post-hoc rigid correction is supported. Camera/dropout closes the previously missing same-policy comparison; it does not change this conclusion.

**Scope and historical notes:** Stage 1 only, full surfaces now, sparse structured touch later. Native loss, exact-grid IoU, pose-adjusted proximity, fitted-object recognition and new-object geometric utility remain separate. Dropout's accurate reconstruction result applies to oracle-aligned fitted identities, not camera-frame conditioning generally. Older plans below are historical unless explicitly reactivated against the pivotal ledger. The original full-dataset checkpoint inventory is optional, not a prerequisite. No separate-attention/voxel-bridge or representation detour is active.
