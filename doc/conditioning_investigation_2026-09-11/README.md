...
**Next GPU check — unseen identities using existing checkpoints:** [UNSEEN_OBJECTS_HANDOFF.md](UNSEEN_OBJECTS_HANDOFF.md) tests all four camera/oracle original/dropout models plus the matched image+pointmap baseline on 32 preselected disjoint objects, two views each. No training. Includes checkpoint replay, per-object native loss, wrong-surface controls and paired comparisons. Local checks pass; GPU execution is pending. This screens the four-object identity-lookup alternative before requesting more training.

**Latest result — visual dropout repairs the tested oracle view gap:** [VISUAL_DROPOUT_RETURNED_FINDINGS.md](VISUAL_DROPOUT_RETURNED_FINDINGS.md) validates both completed runs and exact replay. With full inputs present, reserved loss improves camera .07718 → .05381 and oracle .05785 → .02273. Oracle fitted loss is .02205, so its view gap nearly disappears. Camera/oracle separation widens; alignment remains unresolved. This is a concrete small-task training improvement, not proof of unseen-object geometric transfer or a full-dataset fix.
