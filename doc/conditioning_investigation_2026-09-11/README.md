...
- New files are confined to this folder. The repository ignores this documentation folder; transfer it explicitly to the cluster rather than assuming a normal git push includes it.
...
This requests one H200 for at most one hour, using paths/account/partition from existing jobs. The script is **syntax-checked only** locally; the cluster runtime is needed to execute it. It does not train or overwrite a checkpoint. It writes `outputs/conditioning_probe/train-JOBID.json` (and an intermediate `.partial.json`). Return that JSON and the Slurm log. If paths or resources differ, adapt the launcher to the existing cluster environment.
