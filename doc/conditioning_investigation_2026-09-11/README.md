...
**Next runnable diagnostic — frozen-feature spatial readout:** [FEATURE_READOUT_HANDOFF.md](FEATURE_READOUT_HANDOFF.md) describes `run_feature_readout.sh`, a three-arm short array using the saved representation artifacts. Raw, slot-aware and native-processed features feed matched spatial-query heads; 24 objects fit and eight separate objects are reserved. This tests accessibility of the existing representation without the diffusion network. It is not a sparse-touch completion model or a validated production fix. GPU execution is pending.
