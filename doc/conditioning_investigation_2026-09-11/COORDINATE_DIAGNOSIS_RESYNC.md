...
12 September 2026. This is the current investigation plan. Older proposals below it in the README are historical, not additional pending jobs.
...
The CPU factorization audit executed during resynchronization. The subsequent `fit_frame_factors_gpu.py` runner now implements the two missing coordinate cases and historical input/initial-loss reproduction checks. `FRAME_FACTORS_HANDOFF.md` contains the interactive-terminal command. Python/shell syntax and analyzer checks pass locally; GPU execution is pending. No point-encoder, bridge, sparse-feature or broader-adaptation implementation is the current dependency.
