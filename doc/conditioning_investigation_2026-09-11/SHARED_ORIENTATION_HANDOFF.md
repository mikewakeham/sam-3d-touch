...
Status: the 1000-update run and 112 samples completed; see [SHARED_ORIENTATION_RETURNED_FINDINGS.md](SHARED_ORIENTATION_RETURNED_FINDINGS.md). Target encoding passes, sampled reconstruction fails the planned endpoint even on fitted views. Independent geometry verification is complete. Pose correction helps but residual geometry errors remain, including an all-rigid recall bound below the acceptance criterion. See the returned findings for the proposed next branch. Commands/preflight retries below are historical; do not rerun this job. Overnight implementation remains parked.
...
**Target preflight result and metric correction (13 September):** the returned partial report has zero optimizer updates, exact original-target regeneration for all four objects, and all source hashes match. Twenty-seven of 28 new targets round-trip through the VAE at exactly 100% IoU; one is 99.994085%. Thus the target-encoding check succeeds. Eleven fail the old inter-grid one-voxel precision/recall gate (minimum precision 88.995%, recall remains ≥99.376%). All 28 have exactly 100% two-voxel precision/recall. The old comparison rejected faithful labels because differently oriented voxel grids place occupied centers differently.

For two physical surface voxelizations of the same mesh, a common surface point is at most half a voxel diagonal from either occupied center. Mapping the new cube back with linear map A yields the voxelization distance bound `sqrt(3) * (1 + ||A||_2) * (0.5/64 + 1e-6)`, including the vertex insets used by the two voxelizers. This is 1.577–1.833 original voxels for the 28 targets. It bounds voxelization error, not arbitrary VAE errors; the separate VAE IoU check remains. The common-unit pre-training check therefore now uses two original voxels with ≥95% precision/recall and verifies the analytical bound is below that tolerance. All 28 returned targets pass. This is a pre-training correction based on a positive control; no failed trained candidate is being rescued.

**Cross-arm scoring correction:** use the same two-voxel tolerance in original object units for the new arm and ALL historical controls in the new comparison, including correct/wrong surfaces and oracle reference. Keep their one-voxel measurements separately. Native-target IoU remains unchanged. The common-unit endpoint below now uses two-voxel P/R and F-score, including the raw/rigid witness selection, reference margin and 10 pp surface-dependence screen. Do not mix a new-arm two-voxel result with F12's one-voxel table. Those original F12 comparisons remain valid on their original common grid/criterion.

Evidence: `shared_orientation_target_preflight/returned.json` and `analysis.json`. These are returned GPU target metrics, locally source-checked/recomputed as aggregates; the raw new target supports have not yet been returned for independent array-level reproduction. Sync updated `fit_shared_orientation_gpu.py`, retain the corrected protocol helper, and use `manual_target_grid_fix`. No further diagnostic GPU run is requested separately; the script proceeds to the planned short training once these checks pass.
...
4. Map decoded new targets back with the **known inverse label transform**, and compare with the original decoded target in original object units. Every target must have at least 95% bidirectional proximity within **2/64 original-object units**, and its analytical voxelization bound must be below that tolerance. One-voxel values remain reported but are not a valid absolute acceptance gate for these two grids, as demonstrated by the faithful pre-training targets. All later cross-arm comparisons must use identical physical tolerances.
...
- **Accurate shared-frame endpoint:** reserved mean native-target IoU ≥95% and each object mean ≥90%. In common original units at the calibrated **two-voxel tolerance**, each object's precision and recall must be ≥95% and at most two percentage points below its own decoded-new-target positive control. Preserve comparison with the original oracle reference and camera baseline at that SAME tolerance rather than hiding differences in VAE/discretization floors. One-voxel results remain separately visible.
- **Surface dependence:** correct minus wrong mean **two-voxel** F-score ≥10 pp for each reserved object under the same common-unit scoring procedure. Report raw and measured identity/rigid witness results separately; use maximum minimum TWO-voxel precision/recall among those candidates, symmetrically for every arm/condition. The candidate procedure is inherited from F12, with tolerance explicitly changed before this arm trains. Wrong surfaces contradict visuals; passing is a fitted-surface dependence screen, not new-object utility.
