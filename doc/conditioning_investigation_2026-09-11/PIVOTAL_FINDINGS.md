...
**Surface-only integration:** the user additionally authorized `--no-visual`, now implemented through permanent native visual-modality zeroing in training/validation and checkpoint restoration. `jobs/stage1_full_surface_oracle_no_visual.sh` retains the oracle/full-surface recipe and omits stochastic visual dropout. Thirteen CPU integration/regression tests pass; pretrained CUDA execution has not run locally. This enables the stricter no-visual-interaction diagnostic in [the pointmap isolation plan](POINTMAP_ISOLATION_PLAN.md); it is not a new experimental finding or a claim that conditioning is fixed.
