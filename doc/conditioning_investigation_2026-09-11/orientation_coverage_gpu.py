...
from continue_shared_orientation_gpu import optimizer_digest
from shared_orientation_scope_protocol import additional_shape_parameter
from orientation_coverage_protocol import rotations,indices,rotate_grid,inverse_frame,schedule
...
def read_refs(args):
    ...
    for name,value in probe['source_sha256'].items():assert sha(REPO/name)==value,name
...
def prepare_bank(args,refs,device):
    ...
    report=dict(complete=False,fit_report_sha256=sha(args.fit_dir/'results.json'),probe_report_sha256=sha(args.visual_probe_dir/'results.json'),
        source_sha256={**probe['source_sha256'],**{str(p.relative_to(REPO)):sha(p) for p in [Path(__file__),HERE/'orientation_coverage_protocol.py',HERE/'run_orientation_coverage_pair.py',HERE/'run_shared_orientation_scope_pair.py']}},
...
def fit_arm(args,refs,device):
    ...
    for n,v in bank_report['source_sha256'].items():assert sha(REPO/n)==v,n
    ...
    original=np.load(io.BytesIO(payloads['reference/original_target_occupancy.npy']),allow_pickle=False)
    frames={f['sample_id']:f for f in fit['target_frames']};paired=json.loads((HERE/'camera_dropout_sampling_reference.json').read_text())
