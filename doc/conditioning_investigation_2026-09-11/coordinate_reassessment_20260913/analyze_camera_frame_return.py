...
from analyze_rollout_geometry_local import work
from analyze_frame_probe import summarize as raw_summary
from frame_probe_core import normalization_check
...
def main():
    ...
    rollouts={arm:json.loads((OLD/'rollouts_returned_20260913_143515'/arm/'results.json').read_text()) for arm in ('oracle','constant')}
    losses={arm:json.loads((OLD/'returned_20260913_133239'/arm/'results.json').read_text()) for arm in ('oracle','constant')}
    for name,digest in camera['files_sha256'].items():
        assert Path(name).name==name and sha(a.root/'camera'/name)==digest,name
    for name,digest in camera['source_sha256'].items():assert sha(HERE.parents[2]/name)==digest,name
    ...
    old_geo=OLD/'rollouts_geometry_20260913_143515'
    old_summary=json.loads((old_geo/'summary.json').read_text())
    assert sha(OLD/'analyze_rollout_geometry_local.py')==old_summary['source_sha256']['analyze_rollout_geometry_local.py']
    assert sha(HERE.parent/'pose_shape_geometry.py')==old_summary['source_sha256']['pose_shape_geometry.py']
    for arm in ('oracle','constant'):
        assert sha(OLD/'rollouts_returned_20260913_143515'/arm/'results.json')==old_summary['report_sha256'][arm]
