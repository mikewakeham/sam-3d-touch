...
**Priority order:** the C0–C12 IDs below are not a run queue. See [execution priority](DECISION.md#execution-priority-2026-09-14): assess completed modality-removal checkpoints; verify broader-scope implementation; run a matched short scope comparison; extend only an informative branch. Passed transform checks and future touch/deployment questions do not block that comparison.
...
No new training, inference, dataset mutation, source change, or experiment command is requested by this report. **Updated 2026-09-14:** no-pointmap and no-visual training results are now imported from W&B. All three new runs completed 20 epochs; oracle/no-PM validation loss is 0.0894794, constant/no-PM 0.0890265, and oracle/no-visual 0.1046998. Their checkpoint-level paired losses and generated reconstruction have not been assessed. Their completed training must not be confused with a completed coordinate-mechanism exclusion. See [updated findings](../KEY_FINDINGS.md).
...
|---|---|---|---|
| C0 | Are we diagnosing failed learning, failed generation, or only a similar loss curve? | Full-trained oracle has useful geometry but inaccurate Stage-1 rollouts on the 16+16 bank. New no-PM/no-visual pooled losses are imported; generated reconstructions are not. | **Established for the old oracle checkpoint, unverified for new runs.** Identify checkpoint, split, inputs and endpoint before assigning a failure. No new training needed to establish an existing checkpoint's result. |
...
| C5 | Does differing pointmap/surface frame cause harmful fusion during training? | Pointmap is SSI camera-frame; oracle surface is object-frame with its own normalization. New oracle/no-PM W&B validation loss is 0.0894794; reconstruction is unassessed. | **Open for reconstruction; no pooled-loss rescue observed.** Verified poor reconstruction without PM excludes PM conflict as a necessary/sole cause of that residual. A matched training comparison tests contribution; a shared-frame PM intervention tests the usefulness of that complete preprocessing treatment. None alone proves scale/translation specifically. |
| C6 | Does image/mask perspective conflict with oracle geometry, or cause reliance that obstructs surface learning? | Four-object view/dropout studies; full oracle is worse with visuals removed only at inference. New permanent no-visual run completed with validation loss 0.1046998. | **Reconstruction exclusion still unassessed.** Inference removal does not erase effects of mixed training. Persistent no-visual training structurally excludes sample-dependent visual conflicts under the intended code; validate the restored context once and measure this checkpoint. Constant zero tokens can still affect optimization. |
...
New no-PM and no-visual variants [E15]
  -> training complete; refreshed configs/history/gradients now available
  -> new checkpoint reconstruction not yet measured
  -> cannot yet supply the reconstruction-specific exclusion premises
```
...
- [x] Import completed no-PM/no-visual run configs, histories, recorded source commits and logged steps (2026-09-14 W&B refresh).
- [ ] Verify the actual evaluated checkpoint identity; a W&B run ID is not its weight-file hash. Do not infer its result from the old camera/no-PM run.
