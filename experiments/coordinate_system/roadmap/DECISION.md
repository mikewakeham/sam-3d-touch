...
**Current status, 2026-09-14:** two full-data shared-normalization runs are queued to isolate the remaining pointmap/surface scale-translation hypothesis under the standard object target. Both use no dropout and shape cross-attention. The normalization-only control loads the full surface to calculate its center/radius but withholds surface tokens; the treatment adds the surface tokens. Compare them with the existing stock-normalization image+pointmap and image+pointmap+surface runs as a 2x2 design. The implementation reuses the completed [camera-frame target latent experiment](../scripts/camera_frame_target_latent/README.md), which previously found no clear shared-normalization benefit under camera targets. The separate oracle/no-pointmap/no-dropout run remains the clean orientation upper-bound experiment. Shape-full follow-ups are postponed while these coordinate-specific comparisons run.
