...
def main():
    ...
    ids = json.loads((_source_path(REPO, 'outputs/diagnostics/sample_selection.json')).read_text())['sample_ids']
    ...
    report = {'source_sha256': {str(p.relative_to(REPO)): hashlib.sha256(p.read_bytes()).hexdigest()
                                for p in [Path(__file__), _source_path(REPO, 'train.py'), _source_path(REPO, 'dataloader.py'),
    ...
    (_source_path(HERE, 'frame_factorization_results.json')).write_text(json.dumps(report, indent=2) + '\n')
