...
def main():
    ...
    drop=json.loads((_source_path(HERE, 'visual_dropout_returned_manual/camera/results.json')).read_text())
    config=Path(single['settings']['data_config'])
    if not config.is_absolute():config=_source_path(REPO, config)
