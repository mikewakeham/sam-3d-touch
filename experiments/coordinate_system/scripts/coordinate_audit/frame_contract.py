...
def audit():
    source = _source_path(REPO, "sam3d_objects/pipeline/inference_utils.py")
    ...
    post = _source_path(REPO, "sam3d_objects/model/backbone/tdfy_dit/utils/postprocessing_utils.py")
    ...
    rows = []
    for path in sorted((_source_path(REPO, "data_generation/objaverse-dexonomy/generated_data")).glob("*/object_transform.npz")):
    ...
    (_source_path(HERE, "frame_contract_results.json")).write_text(json.dumps(result, indent=2) + "\n")
