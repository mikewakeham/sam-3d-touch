...
def main():
    ...
    for arm in ['camera', 'oracle']:
        ...
        assert old['driver_sha256'] == sha(_source_path(HERE, 'fit_multiple_views_gpu.py'))
        assert old['view_selector_sha256'] == sha(_source_path(HERE, 'probe_tiny_fit_views.py'))
        ...
        for rel, digest in old['source_sha256'].items():
            assert sha(_source_path(REPO, rel)) == digest, rel
