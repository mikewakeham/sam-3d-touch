...
from experiments.coordinate_system.scripts.tiny_fit.check_tiny_fit import check

HERE = Path(__file__).resolve().parent
ROOT = _source_path(HERE, "tiny_fit_returned_46083371")
...
def main():
    ...
    for arm in ["image", "camera", "oracle"]:
        ...
        for source, digest in report["source_sha256"].items():
            assert hashlib.sha256((_source_path(REPO, source)).read_bytes()).hexdigest() == digest, source
    ...
    (_source_path(HERE, "tiny_fit_summary.json")).write_text(json.dumps(out, indent=2) + "\n")
