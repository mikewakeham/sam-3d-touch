...
sys.path.insert(0,str(next(p for p in Path(__file__).resolve().parents if (_source_path(p, 'train.py')).is_file() and (p / 'sam3d_objects').is_dir())))
import numpy as np
import torch
...
def main():
 ...
 report['source_sha256']={s:hashlib.sha256((_source_path(repo, s)).read_bytes()).hexdigest() for s in ['train.py','dataloader.py','sam3d_objects/model/backbone/dit/embedder/touch.py']}
 def assess(step):
