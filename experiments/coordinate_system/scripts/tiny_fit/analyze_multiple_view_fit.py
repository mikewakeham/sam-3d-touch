...
ROOT = _source_path(HERE, "multiple_view_returned_46083371")
...
def main():
    ...
    for arm, report in reports.items():
        assert report["driver_sha256"] == hashlib.sha256((_source_path(HERE, "fit_multiple_views_gpu.py")).read_bytes()).hexdigest()
        assert report["view_selector_sha256"] == hashlib.sha256((_source_path(HERE, "probe_tiny_fit_views.py")).read_bytes()).hexdigest()
        refpath = _source_path(HERE, "tiny_fit_returned_46083371") / arm / "results.json"
        ...
        for src, digest in report["source_sha256"].items():
            assert hashlib.sha256((next(p for p in Path(__file__).resolve().parents if (_source_path(p, 'train.py')).is_file() and (p / 'sam3d_objects').is_dir()) / src).read_bytes()).hexdigest() == digest, src
        ...
        previous_probe = json.loads((_source_path(HERE, "view_probe_returned_46083371") / (arm + ".json")).read_text())
        for i in range(4):
    ...
    (_source_path(HERE, "multiple_view_summary.json")).write_text(json.dumps(output, indent=2) + "\n")
