...
ROOT = _source_path(HERE, "view_probe_returned_46083371")
...
def main():
    ...
    for arm, report in reports.items():
        assert report["probe_sha256"] == hashlib.sha256((_source_path(HERE, "probe_tiny_fit_views.py")).read_bytes()).hexdigest()
        fit_path = _source_path(HERE, "tiny_fit_returned_46083371") / arm / "results.json"
    ...
    (_source_path(HERE, "view_probe_summary.json")).write_text(json.dumps(summary, indent=2) + "\n")
