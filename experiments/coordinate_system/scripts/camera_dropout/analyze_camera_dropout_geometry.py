...
def main():
    ...
    for name, value in report['reference_sha256'].items(): assert sha(_source_path(HERE, name)) == value
    for name, value in report['source_sha256'].items(): assert sha(_source_path(REPO, name)) == value
    paired = read(_source_path(HERE, 'camera_dropout_sampling_reference.json'))
    drop = read(_source_path(HERE, 'visual_dropout_returned_manual/camera/results.json'))
    ...
    oldroot = _source_path(HERE, 'pose_shape_analysis')
    complete = read(oldroot/'complete.json')
    assert sha(oldroot/'predictions.jsonl') == complete['predictions_sha256']
    assert sha(_source_path(HERE, 'pose_shape_geometry.py')) == complete['config']['source_sha256']['pose_shape_geometry.py']
    ...
    validation = dict(bundle_sha256=sha(args.bundle),members=57,rows=112,
        ...
        source_sha256={n:sha(_source_path(HERE, n)) for n in ('analyze_camera_dropout_geometry.py','pose_shape_geometry.py')},
