...
from experiments.coordinate_system.scripts.alignment_robustness.probe_alignment_tolerance_gpu import locate_dropout, sha
...
def main():
    ...
    paths=[_source_path(HERE, 'tiny_fit_returned_46083371/camera/results.json'),
    ...
    for name,value in drop['source_sha256'].items():assert sha(_source_path(REPO, name))==value,name
    ...
    source_files=[Path(__file__),_source_path(HERE, 'probe_alignment_tolerance_gpu.py'),_source_path(HERE, 'rollout_gpu.py'),
                  _source_path(HERE, 'tiny_fit_gpu.py'),_source_path(HERE, 'bundle_alignment_geometry.py')]
