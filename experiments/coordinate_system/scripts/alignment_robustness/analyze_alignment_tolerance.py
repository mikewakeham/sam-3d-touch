...
from experiments.coordinate_system.scripts.alignment_robustness.alignment_tolerance_protocol import cases, summarize, rotation
...
def analyze(root):
    ...
    multi = json.loads((_source_path(HERE, 'multiple_view_returned_46083371/oracle.json')).read_text())
    drop = json.loads((_source_path(HERE, 'visual_dropout_returned_manual/oracle/results.json')).read_text())
    expected_refs = {name: sha(_source_path(HERE, path)) for name, path in {
