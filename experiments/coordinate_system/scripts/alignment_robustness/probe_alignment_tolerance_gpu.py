...
from experiments.coordinate_system.scripts.alignment_robustness.alignment_tolerance_protocol import cases, rotation
...
def locate_dropout(explicit, reference):
    roots = [explicit] if explicit else [p.parent for p in (_source_path(REPO, 'experiments/coordinate_system/outputs/visual_dropout')).rglob('results.json')]
...
def main():
    ...
    single_path = _source_path(HERE, 'tiny_fit_returned_46083371/oracle/results.json')
    multi_path = _source_path(HERE, 'multiple_view_returned_46083371/oracle.json')
    drop_path = _source_path(HERE, 'visual_dropout_returned_manual/oracle/results.json')
    ...
    assert multi['driver_sha256'] == sha(_source_path(HERE, 'fit_multiple_views_gpu.py'))
    assert multi['view_selector_sha256'] == sha(_source_path(HERE, 'probe_tiny_fit_views.py'))
    for name, value in drop['source_sha256'].items():
        assert sha(_source_path(REPO, name)) == value, name
    ...
    report = {'settings': {'shard': args.shard, 'seed': 29, 'precision': 'bf16', 'steps': 25,
              ...
              'source_sha256': {**drop['source_sha256'], **{str(p.relative_to(REPO)): sha(p) for p in
                  (Path(__file__), _source_path(HERE, 'alignment_tolerance_protocol.py'), _source_path(HERE, 'analyze_alignment_tolerance.py'), _source_path(HERE, 'rollout_gpu.py'), _source_path(HERE, 'tiny_fit_gpu.py'))}},
