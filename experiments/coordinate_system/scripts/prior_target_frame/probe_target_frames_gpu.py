...
from experiments.coordinate_system.scripts.coordinate_audit.frame_contract import rotations, rotate_grid
...
def main():
    ...
    report = {"settings": {k: str(v) if isinstance(v, Path) else v for k, v in vars(a).items()},
              ...
              "source_sha256": {str(q.relative_to(REPO)): sha(q) for q in [source, Path(__file__),
                  _source_path(HERE, "frame_contract.py"), _source_path(REPO, "train.py"), _source_path(REPO, "dataloader.py")]},
