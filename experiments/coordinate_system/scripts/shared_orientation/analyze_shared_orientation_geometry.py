...
from experiments.coordinate_system.scripts.shared_orientation.analyze_shared_orientation_report import main as validate_report
...
def main():
    ...
    assert report==json.loads((_source_path(HERE, 'shared_orientation_returned_manual/results.json')).read_text())
    validate_report()
    ...
    prior=json.loads((_source_path(HERE, 'pose_shape_analysis/complete.json')).read_text())
    assert sha(_source_path(HERE, 'pose_shape_geometry.py'))==prior['config']['source_sha256']['pose_shape_geometry.py']
    assert sha(_source_path(HERE, 'pose_shape_analysis/predictions.jsonl'))==prior['predictions_sha256']
    ...
    for filename in ('camera_dropout_geometry_analysis/predictions.jsonl','pose_shape_analysis/predictions.jsonl'):
        older=[json.loads(s) for s in (_source_path(HERE, filename)).read_text().splitlines()]
