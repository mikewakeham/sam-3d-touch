...
OUT = _source_path(HERE, 'shared_orientation_returned_manual')
...
def main():
    ...
    source = {k: sha(_source_path(REPO, k)) == v for k, v in d['source_sha256'].items()}
    references = {k: sha(_source_path(HERE, k)) == v for k, v in d['reference_sha256'].items()}
    assert all(source.values()) and all(references.values())
    old = json.loads((_source_path(HERE, 'visual_dropout_returned_manual/camera/results.json')).read_text())
    ...
    for file, (model, condition) in zip(historical_paths, selections, strict=True):
        rr = [json.loads(line) for line in (_source_path(HERE, file)).read_text().splitlines()]
    ...
    validation = dict(report_sha256=sha(path), source_matches=source, reference_matches=references,
        historical_analysis_sha256={p: sha(_source_path(HERE, p)) for p in historical_paths},
