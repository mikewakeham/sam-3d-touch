...
from experiments.coordinate_system.scripts.shared.visual_dropout_protocol import dropout_schedule, schedule_digest
...
from experiments.coordinate_system.scripts.shared.pose_shape_geometry import points as support_points, metrics as proximity
from experiments.coordinate_system.scripts.shared_orientation.shared_orientation_protocol import camera_oriented_target, transform_points
...
def main():
    ...
    refs=[_source_path(HERE, 'tiny_fit_returned_46083371/camera/results.json'),
          _source_path(HERE, 'multiple_view_returned_46083371/camera.json'),
          _source_path(HERE, 'visual_dropout_returned_manual/camera/results.json'),
          _source_path(HERE, 'camera_dropout_sampling_reference.json')]
    single,multi,drop,paired=[json.loads(p.read_text()) for p in refs]
    for n,h in drop['source_sha256'].items():assert sha(_source_path(REPO, n))==h,n
    ...
    generator_source=_source_path(REPO, 'data_generation/objaverse-dexonomy/generate_target_latents.py')
    ...
    report=dict(settings=dict(arm='camera_shared_orientation',seed=29,steps=1000,precision='bf16',
        ...
        source_sha256={**drop['source_sha256'],**{str(p.relative_to(REPO)):sha(p) for p in
            [Path(__file__),_source_path(HERE, 'shared_orientation_protocol.py'),generator_source,
             _source_path(HERE, 'pose_shape_geometry.py'),_source_path(HERE, 'rollout_gpu.py'),_source_path(HERE, 'bundle_alignment_geometry.py')]}},
