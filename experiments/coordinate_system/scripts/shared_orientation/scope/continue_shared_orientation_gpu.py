...
from experiments.coordinate_system.scripts.shared_orientation.scope.shared_orientation_scope_protocol import additional_shape_parameter, continuation_schedule, next_noise_seed
...
def main():
    ...
    for name,value in old['source_sha256'].items():assert sha(_source_path(REPO, name))==value,name
    for name,value in old['reference_sha256'].items():assert sha(_source_path(HERE, name))==value,name
    ...
    paired=json.loads((_source_path(HERE, 'camera_dropout_sampling_reference.json')).read_text())
    ...
    report=dict(settings=dict(arm=args.arm,base_step=1000,additional_steps=1000,seed=29,precision='bf16',
        ...
        source_sha256={**old['source_sha256'],**{str(p.relative_to(REPO)):sha(p) for p in
            [Path(__file__),_source_path(HERE, 'shared_orientation_scope_protocol.py'),
             _source_path(REPO, 'sam3d_objects/model/backbone/tdfy_dit/models/mot_sparse_structure_flow.py'),
