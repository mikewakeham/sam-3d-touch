...
import zipfile
import numpy as np
from experiments.coordinate_system.scripts.shared_orientation.analyze_shared_orientation_geometry import analyze, aggregate, witness, same
...
from experiments.coordinate_system.scripts.shared_orientation.scope.shared_orientation_scope_protocol import continuation_schedule, additional_shape_parameter
from experiments.coordinate_system.scripts.shared.visual_dropout_protocol import schedule_digest
...
def main():
    ...
    oldroot=_source_path(HERE, 'shared_orientation_geometry_analysis')
    ...
    for folder in ('camera_dropout_geometry_analysis','pose_shape_analysis'):
        c=json.loads((_source_path(HERE, folder)/'complete.json').read_text());assert sha(_source_path(HERE, folder)/'predictions.jsonl')==c['predictions_sha256']
    ...
    validation=dict(arms=validations,matched_start_and_exposure=True,first_update_loss_identical=True,
        ...
        source_sha256={n:sha(_source_path(HERE, n)) for n in ('analyze_shared_orientation_scope.py','analyze_shared_orientation_geometry.py','pose_shape_geometry.py')})
    ...
    for folder in ('shared_orientation_geometry_analysis','camera_dropout_geometry_analysis','pose_shape_analysis'):
        rr=[json.loads(s) for s in (_source_path(HERE, folder)/'predictions.jsonl').read_text().splitlines()]
