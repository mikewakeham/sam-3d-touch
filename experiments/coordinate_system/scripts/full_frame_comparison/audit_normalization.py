...
DATA = _source_path(REPO, 'data_generation/objaverse-dexonomy')
INV = HERE.parent
...
def main():
    ...
    src = _source_path(REPO, 'sam3d_objects/model/backbone/dit/embedder/touch.py')
    ...
    rollout_root = _source_path(INV, 'oracle_upper_bound/rollouts_returned_20260913_143515/oracle')
    ...
    output = {'summary': summary, 'rows': rows, 'same_object_view_pairs': pairs,
              'source_sha256': {str(p.relative_to(REPO)):sha(p) for p in [Path(__file__), src, manifest, _source_path(REPO, 'train.py'), _source_path(REPO, 'dataloader.py')]},
    ...
    (_source_path(HERE, 'normalization_audit.json')).write_text(json.dumps(output,indent=2)+'\n')
