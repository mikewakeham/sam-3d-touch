...
from experiments.coordinate_system.scripts.full_frame_comparison.frame_probe_core import check_observation, check_report_pair, check_loss_pair, normalization_check, sampled_mesh_check
...
def reuse_audited_geometry(args, datasets, references, report):
    ...
    assert sha(_source_path(REPO, 'data_generation/objaverse-dexonomy/generate_target_latents.py')) == saved['target_generation_source_sha256']
...
def geometry_reference(args, datasets, references, report, device):
    ...
    import trimesh
    path = _source_path(REPO, 'data_generation/objaverse-dexonomy/generate_target_latents.py')
...
def main():
    ...
    for name,digest in ref['source_sha256'].items():
        assert sha(_source_path(REPO, name))==digest, f'Historical source changed: {name}'
    for name,digest in roll['additional_source_sha256'].items():
        assert sha(_source_path(REPO, name))==digest, f'Historical rollout helper changed: {name}'
    ...
    report={'complete':False,'settings':{**{k:str(v) if isinstance(v,Path) else v for k,v in vars(args).items()},'arm':'camera'},
            ...
            'additional_source_sha256':{str(p.relative_to(REPO)):sha(p) for p in (Path(__file__),_source_path(HERE, 'frame_probe_core.py'))},
