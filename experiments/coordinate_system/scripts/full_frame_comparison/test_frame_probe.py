...
import numpy as np
from experiments.coordinate_system.scripts.full_frame_comparison.frame_probe_core import check_observation, check_report_pair, check_loss_pair, normalization_check
from experiments.coordinate_system.scripts.full_frame_comparison.analyze_frame_probe import summarize

sys.path.insert(0, str(Path(__file__).resolve().parent.parent))
from experiments.coordinate_system.scripts.tools.evidence import Evidence

ROOT='doc/conditioning_investigation_2026-09-11/oracle_upper_bound'


def fixture():
    evidence = Evidence()
    try:
        rollouts={a:json.loads(evidence.read(f'{ROOT}/rollouts_returned_20260913_143515/{a}/results.json')) for a in ('oracle','constant')}
        losses={a:json.loads(evidence.read(f'{ROOT}/returned_20260913_133239/{a}/results.json')) for a in ('oracle','constant')}
    finally:
        evidence.close()
