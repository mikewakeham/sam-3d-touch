...
from experiments.coordinate_system.scripts.native_representation.representation_geometry import point_grid, overlap, sample_mesh, surface_agreement, zero_surface
...
def main():
    ...
    source_path = _source_path(REPO, 'data_generation/objaverse-dexonomy/generate_target_latents.py')
    ...
    report = {'settings': {k: str(v) if isinstance(v, Path) else v for k, v in vars(args).items()},
        ...
        'source_sha256': {str(p.relative_to(REPO)): sha(p) for p in [Path(__file__), _source_path(HERE, 'representation_geometry.py'),
            source_path, _source_path(REPO, 'dataloader.py'), _source_path(REPO, 'sam3d_objects/model/backbone/dit/embedder/touch.py'),
            _source_path(REPO, 'sam3d_objects/model/backbone/dit/embedder/vecsetx/autoencoder.py'),
            _source_path(REPO, 'sam3d_objects/model/backbone/dit/embedder/vecsetx/utils.py'),
            _source_path(REPO, 'sam3d_objects/model/backbone/tdfy_dit/models/sparse_structure_vae.py')]},
