"""Bounded 16-object training comparison with distinct held views/objects."""
...
from experiments.coordinate_system.scripts.identity_transfer.object_transfer_protocol import MODEL_KEYS, digest_json, make_schedule, validate_plan
...
def main():
    ...
    plan_path = _source_path(HERE, 'object_transfer_plan.json')
    plan = json.loads(plan_path.read_text()); validate_plan(plan)
    seed, precision = plan['seed'], 'bf16'
    reference_path = _source_path(HERE, 'tiny_fit_returned_46083371/oracle/results.json')
    reference = json.loads(reference_path.read_text())
    for source, digest in reference['source_sha256'].items():
        assert sha(_source_path(REPO, source)) == digest, source
    ...
    report = dict(settings={'model_key': args.model_key, 'seed': seed, 'precision': precision, 'steps': 1024,
                  ...
                  reference_sha256=sha(reference_path), source_sha256={**reference['source_sha256'], **{
                      str(p.relative_to(REPO)): sha(p) for p in (Path(__file__), _source_path(HERE, 'object_transfer_protocol.py'))}},
                  ...
                  scope='16 fitted identities: four fitted and two held views each. 16 held identities, '
                        ...
                        '1024 updates, 256 presentations per fitted identity. Not an isolated dataset-size effect.')
    args.output_dir.mkdir(parents=True)

    def save():
        (args.output_dir/'results.partial.json').write_text(json.dumps(report, indent=2)+'\n')

    def forward_loss(index, drop=False, swap=False, per_object=False):
        ...
        handle = None
        if drop and not report['dropout_context_verified']:
            def check_context(module, inputs):
                ...
                assert torch.count_nonzero(context[:, visual.shape[1]:]).item() > 0
                report['dropout_context_verified'] = True
                ...
                for swap in ([False, True] if swaps and encoder is not None else [False]):
                    ...
                    for draw in range(draws):
                        ...
                        with amp(device, precision):
                            native, values = forward_loss(index, swap=swap, per_object=True)
    ...
    assert args.model_key != 'oracle_dropout' or report['dropout_context_verified']
