...
def main():
    ...
    parser.add_argument('--fit-root', type=Path, default=Path('experiments/coordinate_system/outputs/object_transfer/manual'))
    ...
    archived = _source_path(HERE, 'object_transfer_returned_manual') / args.model_key / 'results.json'
    ...
    assert sha(_source_path(HERE, 'object_transfer_plan.json')) == fit['plan_sha256']
    ...
    report = dict(settings={'model_key': args.model_key, 'fit_root': str(args.fit_root),
                  ...
                  source_sha256={**fit['source_sha256'], **{str(p.relative_to(REPO)): sha(p) for p in (
                      Path(__file__), _source_path(HERE, 'analyze_object_transfer.py'), _source_path(HERE, 'analyze_transfer_checkpoints.py'))}},
