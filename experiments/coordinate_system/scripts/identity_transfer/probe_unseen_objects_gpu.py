...
def main():
    ...
    for name, expected in trained['source_sha256'].items():
        assert sha(_source_path(REPO, name)) == expected, name
    if policy == 'dropout':
    ...
    if checkpoint is None:
        if policy == 'dropout':
            checkpoint = _source_path(REPO, f'experiments/coordinate_system/outputs/visual_dropout/manual/{arm}/checkpoint_1000.pt')
        else:
    ...
    report = dict(settings={'model_key': args.model_key, 'checkpoint': str(checkpoint), 'draws': 8,
                  ...
                  source_sha256={str(p.relative_to(REPO)): sha(p) for p in
                                 (Path(__file__), _source_path(REPO, 'train.py'), _source_path(REPO, 'dataloader.py'),
