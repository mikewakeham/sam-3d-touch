...
from experiments.coordinate_system.scripts.identity_transfer.object_transfer_protocol import digest_json, make_schedule, validate_plan
from experiments.coordinate_system.scripts.identity_transfer.analyze_object_transfer import validate
...
def main():
    ...
    baseline_path = _source_path(HERE, 'object_transfer_returned_manual/oracle/results.json')
    ...
    for name, digest in baseline['source_sha256'].items():
        assert sha(_source_path(REPO, name)) == digest, name
    ...
    report = dict(settings={'model_key': args.model_key, 'seed': seed, 'precision': precision, 'steps': 1024,
                  ...
                  reference_sha256=sha(reference_path), source_sha256={**baseline['source_sha256'], **{
                      str(p.relative_to(REPO)): sha(p) for p in (Path(__file__), _source_path(HERE, 'analyze_object_transfer.py'),
                                                               _source_path(HERE, 'analyze_constant_surface.py'))}},
