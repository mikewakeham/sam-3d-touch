...
def locate_fit(explicit, reference):
    if explicit is not None:
        ...
        candidates = []
        for root in (_source_path(REPO, 'experiments/coordinate_system/experiments/coordinate_system/outputs/multiple_view_fit'),
                     _source_path(REPO, 'experiments/coordinate_system/outputs/multiple_view_fit')):
            if root.exists():
...
def main():
    ...
    assert multi['driver_sha256'] == digest(_source_path(HERE, 'fit_multiple_views_gpu.py'))
    assert multi['view_selector_sha256'] == digest(_source_path(HERE, 'probe_tiny_fit_views.py'))
    ...
    for source, expected in single['source_sha256'].items():
        assert digest(_source_path(REPO, source)) == expected, source
    ...
    report = dict(settings={'fit_dir': str(fit_dir), 'arm': 'oracle', 'draws': 8,
                  ...
                  source_sha256={str(p.relative_to(REPO)): digest(p) for p in
                                 (Path(__file__), _source_path(HERE, 'probe_tiny_fit_views.py'),
                                  _source_path(REPO, 'sam3d_objects/model/backbone/dit/embedder/embedder_fuser.py'))},
