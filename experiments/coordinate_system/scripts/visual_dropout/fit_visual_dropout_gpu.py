...
from experiments.coordinate_system.scripts.visual_dropout.probe_oracle_visual_streams_gpu import locate_fit
...
def main():
    ...
    for source, expected in single['source_sha256'].items():
        assert sha(_source_path(REPO, source)) == expected, source
    ...
    report = dict(settings={**multi['settings'], 'visual_dropout_fraction': .5,
                  ...
                  source_sha256={**single['source_sha256'], **{str(p.relative_to(REPO)): sha(p) for p in
                      (Path(__file__), _source_path(HERE, 'visual_dropout_protocol.py'), _source_path(HERE, 'probe_oracle_visual_streams_gpu.py'),
                       _source_path(HERE, 'probe_tiny_fit_views.py'))}},
